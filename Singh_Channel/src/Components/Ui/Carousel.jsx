...
function Carousel({ autoSlide = false, autoSlideInterval = 3000, items }) {
  ...
  return (
    <div className="relative h-[340px] w-full max-w-5xl overflow-hidden rounded-lg bg-white shadow-lg sm:h-[300px]">
      ...
      <Button
        variant="overlay"
        onClick={() => paginate(-1)}
        className="absolute top-1/2 left-2 z-10 -translate-y-1/2 !hidden md:!flex"
        iconLeft={<ChevronLeft size={24} />}
      />
      <Button
        variant="overlay"
        onClick={() => paginate(1)}
        className="absolute top-1/2 right-2 z-10 -translate-y-1/2 !hidden md:!flex"
